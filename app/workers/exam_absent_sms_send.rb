class ExamAbsentSmsSend < Struct.new(:exam)
  ...
  def send_sms(exam)
    exam.exam_catlogs.includes([:student]).only_absents.in_groups_of(50).each_with_index do |exam_catlog_group, index|
      ...
      #"https://www.txtguru.in/imobile/api.php?username=swapnilpatil04&password=95183992&source=update&dmobile=#{exam_catlog_group.compact.map(&:student).map(&:p_mobile).join(',')}&message=#{message}"
